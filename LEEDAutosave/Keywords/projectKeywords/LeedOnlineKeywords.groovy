...
class LeedOnlineKeywords {
	...
	def get_rowNumber_of_form(String form_name, String ratingSys){
		...
		for (Row row : sheet) {
			...
			for(Cell cell:row){
				if (cell.getCellType() == Cell.CELL_TYPE_STRING) {
					if (cell.getRichStringCellValue().getString().trim().equals(ratingSys) && flag==1) {
						println row.getRowNum();
						rowNum1 = row.getRowNum();
						return row.getRowNum();
					}
				}
			}
	...
	def public String readPdf(){
		...
		String pdf = RunConfiguration.getProjectDir()+"/Include/TestData/DownloadedFiles/creditForm.pdf"
